...
export function useNotes() {
  ...
  const getNotesForThread = useCallback(async (threadId: string) => {
    setIsLoading(true);
    setError(null);
    try {
  ...
  const hasNotes = useCallback((threadId: string) => {
    return notes.some(note => note.threadId === threadId);
  }, [notes]);

  const addNote = useCallback(async (threadId: string, content: string, color: string = 'default'): Promise<Note | null> => {
    setIsLoading(true);
    setError(null);
    try {
  ...
  const editNote = useCallback(async (noteId: string, content: string): Promise<Note | null> => {
    setIsLoading(true);
    setError(null);
    try {
  ...
  const togglePinNote = useCallback(async (noteId: string): Promise<Note | null> => {
    setIsLoading(true);
    setError(null);
    try {
      ...
      if (updatedNote) {
        setNotes(prev => prev.map(note => note.id === noteId ? updatedNote : note));
        const pinStatus = updatedNote.isPinned ? t("common.notes.notePinned") : t("common.notes.noteUnpinned");
  ...
  const changeNoteColor = useCallback(async (noteId: string, color: string): Promise<Note | null> => {
    setIsLoading(true);
    setError(null);
    try {
